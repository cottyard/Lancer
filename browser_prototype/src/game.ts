class Game
{
    ...
    layout_1st = ['Archer', 'Wagon', 'Archer', 'Rider', 'King', 'Rider', 'Archer', 'Wagon', 'Archer'];
    ...
    run()
    {
        ...
        halo_map[1].set('Rider', [Angle.create(Direction.DownLeftRight, GameCanvas.halo_size_small), Angle.create(Direction.DownRightLeft, GameCanvas.halo_size_small)]);
        ...
        halo_map[2].set('Rider', [Angle.create(Direction.UpLeftRight, GameCanvas.halo_size_small), Angle.create(Direction.UpRightLeft, GameCanvas.halo_size_small)]);
        ...
        for (let i = 0; i < grids; ++i) {
            ...
            if (f !== undefined)
            {
                f.call(this.canvas, p, g.const.STYLE_RED_LIGHT, halo_map[1].get(name));
            }
        }
        for (let i = 0; i < grids; ++i) {
        ...
        for (let i = 0; i < grids; ++i) {
            ...
            if (f !== undefined)
            {
                f.call(this.canvas, p, g.const.STYLE_BLUE_LIGHT, halo_map[2].get(name));
            }
        }
        for (let i = 0; i < grids; ++i) {
